...
def get_dataset(dataset_id, path=None):
	...
	if dataset_id == 'darkcars':
		return DarkCars(path)

	if dataset_id == 'sony_SITD':
		return Sony_SITD(path)
...
class Sony_SITD(SITD):
	def __init__(self, base_dir=None):
		super().__init__(base_dir)
		self.__imgs_dir = self._base_dir
		self.__identity_map_path = os.path.join(self._base_dir,  'Sony.txt')
	...
	def __list_imgs(self):
		...
		return img_paths , class_ids , contant_ids

	def read_images(self):
		img_paths , class_ids , contant_ids = self.__list_imgs()

		unique_class_ids = list(set(class_ids))
		unique_contant_ids = list(set(contant_ids))
		...
		for i in range(len(img_paths)):
			...
			contents[i] = unique_contant_ids.index(contant_ids[i])

		return imgs, classes, contents


class DarkCars(DataSet):
	def __init__(self,base_dir):
		super().__init__(base_dir)
		self.__imgs_dir = os.path.join(self._base_dir, 'background')

	def __list_imgs(self):
		img_paths = []
		class_ids = []
		contant_ids = []
		for img_path in glob.glob(str(self.__imgs_dir) + '/*.JPG'):
			img_paths.append(img_path)
			class_ids.append(int(os.path.split(img_path)[-1].split('_')[0]))
			contant_ids.append("".join(os.path.split(img_path)[-1].split('_')[-1].split('.')[0:-1]))
